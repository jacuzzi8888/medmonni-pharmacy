...
// Hardcoded data from src/data/products.ts to avoid import issues
const PRODUCTS = [
    {
        id: "p1",
...
function generateMigrationSql() {
    ...
    sql += `INSERT INTO products (name, slug, price, category, image, description, paystack_link, is_active, in_stock, is_featured) VALUES\n`;
    ...
    const values = PRODUCTS.map(p => {
        ...
        const image = p.img.replace(/'/g, "''");
        const description = (p.description || '').replace(/'/g, "''");
        const paystackLink = p.paystackLink.replace(/'/g, "''");
        const isFeatured = !!p.badge;

        return `('${name}', '${slug}', ${price}, '${category}', '${image}', '${description}', '${paystackLink}', true, true, ${isFeatured})`;
    });

    sql += values.join(',\n');
    sql += `\nON CONFLICT (slug) DO NOTHING;`;

    const outputPath = path.join(__dirname, 'migrate-products.sql');
